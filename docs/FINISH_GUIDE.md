...
| Upload shows “Network error” on Vercel (works locally) | Two causes, both fixed: (1) PDF parsing crashed on Vercel with `ReferenceError: DOMMatrix is not defined` — the old pdf-parse library needed browser APIs / a native canvas binary that cannot load in serverless; switched to `unpdf` (serverless-native, zero dependencies). (2) Vercel rejects request bodies > ~4.5 MB before the app runs — the form now pre-checks and explains this; split/compress larger PDFs or self-host to keep the 10 MB limit |
